...
void main() {
  group('WeatherCubit', () {
    ...
    setUp(() async {
      ...
      ).thenAnswer((_) async => optionOf(weather));
    ...
    test('initial state is correct', () {
      mockHydratedStorage(() {
        ...
        weatherCubit.state.maybeWhen(
          initial: (unit, weather) {
            expect(unit, TemperatureUnits.celsius);
            expect(weather, Weather.empty);
          },
          orElse: () => fail('Not initial state'),
        );
      });
    });
    ...
    group('fetchWeather', () {
      ...
      blocTest<WeatherCubit, WeatherState>(
        'emits [loading, failure] when getWeather return none',
        setUp: () {
          when(
            () => weatherRepository.getWeather(any()),
    ...
    group('refreshWeather', () {
      ...
      blocTest<WeatherCubit, WeatherState>(
        'emits nothing when location is empty',
        build: () => weatherCubit,
        seed: () => WeatherState.success(weather: Weather.empty),
        act: (cubit) => cubit.refreshWeather(),
        expect: () => <WeatherState>[],
        verify: (_) {
          verifyNever(() => weatherRepository.getWeather(any()));
      ...
      blocTest<WeatherCubit, WeatherState>(
        'emits nothing when weather returns none',
        setUp: () {
          when(
            () => weatherRepository.getWeather(any()),
          ).thenAnswer((_) async => none());
      ...
      blocTest<WeatherCubit, WeatherState>(
        ...
        expect: () => <Matcher>[
          isA<WeatherState>().having(
            (w) => w.mapOrNull(success: (val) => val.weather),
            'weather',
            isA<Weather>()
                .having((w) => w.lastUpdated, 'lastUpdated', isNotNull)
                .having((w) => w.condition, 'condition', weatherCondition)
